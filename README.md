# treeSQL
An easy and accessible way to apply SQL queries on tree structures like JSON, XML or YAML. It uses nearly unmodified SQL Syntax and produces tables as it's primary result format.
...
As you can see, we use an **rJP** with **multiple range literals**. Each ~literal is like a key and with multiple literals, you get something similar as an **SQL composite key**.
...
If such access does not return a value, there is still a table result entry but it uses SQL's **NULL** to show missing values.

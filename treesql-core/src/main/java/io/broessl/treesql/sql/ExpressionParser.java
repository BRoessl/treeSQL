...
public final class ExpressionParser implements ParseTreeListener {
  ...
  private static final List<Integer> LITERAL_TYPES =
      List.of(
          TreeSQLLexer.NUMERIC_LITERAL,
          TreeSQLLexer.STRING_LITERAL,
          TreeSQLLexer.NULL,
          TreeSQLLexer.TRUE,
          TreeSQLLexer.FALSE,
          TreeSQLLexer.RANGED_LITERAL,
          TreeSQLLexer.JSON_TEXT_VALUE,
          TreeSQLLexer.VALUE_AT_LITERAL,
          TreeSQLLexer.PATH_VARIABLE);

  public List<Stackable> getStack() {
    while (!operatorStack.isEmpty()) {
      TerminalNode topOperator = operatorStack.removeLast();
      StackOperation operation =
          OperationRegistry.getInstance().getOperation(topOperator.getText());

...
public class TransientScanContext implements ScanContext {
	...
	public TransientScanContext(TransientScanContext previousBindings, TreeNodeIdentifier stepOrNodeLiteral) {
		...
		if (parentContext != null && parentContext.getBinding(rangeLiteral) != null) {
			...
			if (alreadyBound != null) {
				throw new IllegalStateException(
						String.format("You can not add binding '%s' to '%s' since it is already bound to '%s'",
	...
	@Override
	public ScanContext chainWithPathBinding(TreeNodeIdentifier stepOrNodeLiteral, String rangeLiteral,
			NavigableTreeNode valueAt) {
	...
	public String getEvaluationPath() {
		if (parentContext == null) {
			...
			return parentContext.getEvaluationPath()
					+ "/" + stepName.nativeValue().toString();
		}
	}
	...
	private static class SolidifiedJsonScanBindings implements ScanContext {
		...
		@Override
		public ScanContext chainWithPathBinding(TreeNodeIdentifier forNodeName, String bindingName,
				NavigableTreeNode valueAt) {
			throw new UnsupportedOperationException("SolidifiedJsonScanBindings can not be chained");
		}

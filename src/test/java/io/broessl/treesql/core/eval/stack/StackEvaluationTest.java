...
public class StackEvaluationTest {
    ...
    void testEvaluateComplexC() {
        List<Stackable> expressionStack = ExpressionParser.parseExpressionStack("SIN((1 + 2) * (3 - 4))"); // sin of -3
                                                                                                           // is
                                                                                                           // -0.1411200081
        var result = new StackEvaluation(expressionStack).evaluate(null);
        Assertions.assertEquals(-0.1411200081, ((TreeNumber) result).nativeValue().doubleValue(), 0.0000000001);
    }

    @Test
    void testEvaluateWithContext() {
        ScannableTreeNode patternScan = ScannableTreeNode
                .forRoot(NavigableJsonNode.linkRoot(TestWithJsonData.testDataSimpleDataTree()));
        ...
        List<Stackable> expressionStackA = ExpressionParser
                .parseExpressionStack("'FOO' + level_1 + level_2 + level_3");
        ...
        List<Stackable> expressionStackD = ExpressionParser.parseExpressionStack("~level_2 == '/highly/nested'");

...
class ScannableTreeNodeTest extends TestWithJsonData {
	...
	void testSimpleScanNoMatch() {
		ScannableTreeNode node = ScannableTreeNode.forRoot(NavigableJsonNode.linkRoot(testDataSimpleDataTree()));
	...
	void testSimpleRootMatch() {
		...
		List<ScannableTreeNode> scanResult = node.scan("").toList();
		Assertions.assertEquals(1, scanResult.size());
		Assertions.assertEquals("{\"foo\":[\"bar\",\"baz\"],\"highly\":{\"nested\":{\"objects\":true}}}",
				scanResult.get(0).toString());
	}

	@Test
	void testSimpleScanArray() {
		ScannableTreeNode node = ScannableTreeNode.forRoot(NavigableJsonNode.linkRoot(testDataSimpleDataArray()));
	...
	void testSimpleScanAfterScan() {
		ScannableTreeNode firstScan = ScannableTreeNode.forRoot(NavigableJsonNode.linkRoot(testDataSimpleDataTree()));
		;
	...
	void testSimpleBackScanAfterScan() {
		...
		Assertions.assertEquals("{\"objects\":true}", secondScan.toString());
		Assertions.assertEquals("/highly/nested/objects/~..", secondScan.getContext().getEvaluationPath());
		Assertions.assertEquals("/highly/nested", secondScan.absolutePath());
	}

	@Test
	void testSimpleSideScan() {
		ScannableTreeNode firstScan = ScannableTreeNode
				.forRoot(NavigableJsonNode.linkRoot(testDataArrayWithTenIntegers()));
	...
	void testSimpleSideScanIntermediate() {
		ScannableTreeNode firstScan = ScannableTreeNode.forRoot(NavigableJsonNode.linkRoot(testDataSimpleDataArray()));
	...
	@Test
	void testBindings() {
		ScannableTreeNode patternScan = ScannableTreeNode
				.forRoot(NavigableJsonNode.linkRoot(testDataSimpleDataTree()));
		...
		Assertions.assertEquals("/highly/nested", scanResult.get(0).getContext().getBinding("~level_2"));
		Assertions.assertEquals("/highly/nested/objects", scanResult.get(0).getContext().getBinding("~level_3"));
		Assertions.assertEquals(new TreeString("highly"), scanResult.get(0).getContext().getBinding("level_1"));
		Assertions.assertEquals(new TreeString("nested"), scanResult.get(0).getContext().getBinding("level_2"));
		Assertions.assertEquals(new TreeString("objects"), scanResult.get(0).getContext().getBinding("level_3"));
		NavigableTreeNode ntn = Assertions.assertInstanceOf(NavigableTreeNode.class, scanResult.get(0).getContext().getBinding("@level_3"));
	...
	void testBindingsBack() {
		ScannableTreeNode patternScan = ScannableTreeNode
				.forRoot(NavigableJsonNode.linkRoot(testDataSimpleDataTree()));
		List<ScannableTreeNode> scanResult = patternScan.scan("/~level_1/~level_2/~level_3/..~back_to_level_2")
				.toList();
		Assertions.assertEquals(1, scanResult.size());
		Assertions.assertEquals("{\"objects\":true}", scanResult.get(0).toString());
		Assertions.assertEquals("/highly", scanResult.get(0).getContext().getBinding("~level_1"));
		Assertions.assertEquals("/highly/nested", scanResult.get(0).getContext().getBinding("~level_2"));
		Assertions.assertEquals("/highly/nested/objects", scanResult.get(0).getContext().getBinding("~level_3"));
		Assertions.assertEquals("/highly/nested/objects/~..",
				scanResult.get(0).getContext().getBinding("~back_to_level_2"));
		Assertions.assertEquals(scanResult.get(0).getContext().getBinding("@back_to_level_2"),
				scanResult.get(0).getContext().getBinding("@level_2"));
	}

	@Test
	void testFileScan() {
		NavigableDirectory squealableFileSystem = new NavigableDirectory(
				SquealableFileSystemTest.TEST_DIR.toPath());
		...
		Assertions.assertTrue(scanResult.contains("/subFolder/sub~SubFolder/testA.json/#json/0"));
	}
	...
	void testFileScanWithTiled() {
		NavigableDirectory squealableFileSystem = new NavigableDirectory(
				SquealableFileSystemTest.TEST_DIR.toPath());
		ScannableTreeNode fileScan = ScannableTreeNode.forRoot(squealableFileSystem);
		ScannableTreeNode scanResult = fileScan.scan("/subFolder/sub~1SubFolder/testA.json/#json/0").findFirst().get();
		Assertions.assertEquals("/subFolder/sub~SubFolder/testA.json/#json/0", scanResult.absolutePath());
	}
